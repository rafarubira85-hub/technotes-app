...
import { X, Save, AlertTriangle, User, Tag, FileText } from 'lucide-react';

export default function EditNoteModal({ isOpen, onClose, note, onUpdateNote }) {
  ...
  useEffect(() => {
    if (note) {
      ...
      setCategory(note.category || 'mantenimiento');
      setTechnicianName(note.technician_name || localStorage.getItem('tech_name') || '');
    }
  }, [note]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.message || 'Error al actualizar el aviso');
    } finally {
    ...
    <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-xl max-w-lg w-full overflow-hidden border border-slate-200">
        ...
        <div className="bg-slate-900 text-white p-4 flex items-center justify-between">
          <div>
            <h3 className="font-bold text-lg">Editar Aviso Pendiente</h3>
            <p className="text-xs text-slate-300">Modifique los detalles del aviso seleccionado</p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4 text-xs md:text-sm">
          ...
          <div>
            <label className="block font-semibold text-slate-700 mb-1 flex items-center gap-1">
              <User className="w-3.5 h-3.5 text-slate-500" /> Técnico:
            </label>
            <input
              type="text"
